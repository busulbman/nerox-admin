...
export default function SuperAdminPage() {
  ...
  async function handleCreateRestaurant(event: FormEvent<HTMLFormElement>) {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.adminEmail.trim())) {
      setFeedback({ tone: 'error', text: 'Geçerli bir e-posta adresi girin.' })
      return
    }
            ...
            {modal.type === 'email-change' && (
              <div className="mt-5 space-y-4">
                ...
                <div className="rounded-lg border border-amber-400/30 bg-amber-400/10 px-3 py-2.5">
                  <p className="text-xs font-semibold text-amber-300">Auth e-postası değişmezse giriş e-postası değişmez.</p>
                  <p className="mt-1 text-xs text-amber-200/80">
                    Bu işlem yalnızca panelde görünen e-postayı günceller (bekleyen değişiklik olarak kaydedilir).
                    İşletmenin giriş yaptığı e-postanın da değişmesi için Firebase Console → Authentication üzerinden
                    veya kullanıcının kendi hesabından güncellenmesi gerekir.
                  </p>
                </div>
