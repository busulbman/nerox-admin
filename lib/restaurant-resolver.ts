...
export async function resolveRestaurantBySlugOrId(slugOrId: string): Promise<ResolvedRestaurant | null> {
  ...
  for (const restaurantDoc of allRestaurantsSnap.docs) {
    const generalSettingsRef = doc(db, 'restaurants', restaurantDoc.id, 'settings', 'general')
    ...
    if (generalSettingsSnap.exists()) {
      ...
      if (typeof data.slug === 'string' && data.slug.toLowerCase() === slugOrId.toLowerCase()) {
        return {
          id: restaurantDoc.id,
          slug: data.slug,
          name: typeof data.businessName === 'string' ? data.businessName : null,
  ...
  if (slugOrId === RESTAURANT_ID || slugOrId === 'varina') {
    return {
      id: RESTAURANT_ID,
      slug: null,
      name: null,
